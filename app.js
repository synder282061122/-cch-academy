...
const I18N = {
 ...
 tab_warning_title:" ตรวจพบการสลับหน้าจอ",
 ...
 tab_warning_title:" Tab Switch Detected",
...
function hideSplash(){
 ...
 if(s){ s.style.transition='opacity .3s'; s.style.opacity='0'; setTimeout(()=>s.remove(),320); }
}
function showLogin(){
 hideSplash();
...
function renderAdminSettings(c){
 const savedGasUrl = localStorage.getItem('cch_gas_url')||'';
 const savedSheetUrl = localStorage.getItem('cch_sheet_url')||'';
 const savedCourseGasUrl = localStorage.getItem('cch_course_gas_url')||'';
 const savedCourseSheetUrl = localStorage.getItem('cch_course_sheet_url')||'';
 const sheetLink = savedSheetUrl ? `<a href="${escapeHtml(savedSheetUrl)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:5px;color:var(--teal);font-size:12.5px;font-weight:700;margin-bottom:10px;text-decoration:none;"> ${LANG==='th'?'เปิด Sheet พนักงาน':'Open Staff Sheet'} </a>` : '';
 const courseSheetLink = savedCourseSheetUrl ? `<a href="${escapeHtml(savedCourseSheetUrl)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:5px;color:var(--teal);font-size:12.5px;font-weight:700;margin-bottom:10px;text-decoration:none;"> ${LANG==='th'?'เปิด Sheet คอร์ส':'Open Course Sheet'} </a>` : '';

 c.innerHTML = `
 <div class="card">
 <div style="font-weight:800;color:var(--navy);margin-bottom:4px;"> ${LANG==='th'?'ซิงค์รายชื่อพนักงาน':'Sync Staff List'}</div>
 <div style="font-size:11px;color:var(--gray);margin-bottom:10px;font-family:monospace;background:var(--sand);padding:8px;border-radius:8px;">nameTH | nameEN | username | password | department | level | role | active</div>
 <div class="inputGroup"><label> Sheet URL</label>
 <input id="sheetUrlInput" value="${escapeHtml(savedSheetUrl)}" placeholder="https://docs.google.com/spreadsheets/d/..." style="font-size:12px;"></div>
 ${sheetLink}
 <div class="inputGroup"><label> Apps Script Web App URL</label>
 <input id="gasUrlInput" value="${escapeHtml(savedGasUrl)}" placeholder="https://script.google.com/macros/s/XXXX/exec" style="font-size:12px;"></div>
 <div style="display:flex;gap:8px;">
 <button class="btn btn-gray" style="flex:1;font-size:13px;" onclick="saveSheetUrls()">${LANG==='th'?'บันทึก':'Save'}</button>
 <button class="btn btn-teal" style="flex:1;font-size:13px;" onclick="syncFromGoogleSheet()"> ${LANG==='th'?'ซิงค์พนักงาน':'Sync Staff'}</button>
 </div>
 <div id="syncStatus" style="margin-top:8px;font-size:12px;color:var(--gray);"></div>
 </div>

 <div class="card">
 <div style="font-weight:800;color:var(--navy);margin-bottom:4px;"> ${LANG==='th'?'ซิงค์คอร์สเรียน':'Sync Courses'}</div>
 <div style="font-size:11px;color:var(--gray);margin-bottom:10px;font-family:monospace;background:var(--sand);padding:8px;border-radius:8px;">deptNameTH | deptNameEN | deptIcon | titleTH | titleEN | driveFileId | durationSec | requiredLevel</div>
 <div class="inputGroup"><label> Sheet URL</label>
 <input id="courseSheetUrlInput" value="${escapeHtml(savedCourseSheetUrl)}" placeholder="https://docs.google.com/spreadsheets/d/..." style="font-size:12px;"></div>
 ${courseSheetLink}
 <div class="inputGroup"><label> Apps Script Web App URL</label>
 <input id="courseGasUrlInput" value="${escapeHtml(savedCourseGasUrl)}" placeholder="https://script.google.com/macros/s/XXXX/exec" style="font-size:12px;"></div>
 <div style="display:flex;gap:8px;">
 <button class="btn btn-gray" style="flex:1;font-size:13px;" onclick="saveSheetUrls()">${LANG==='th'?'บันทึก':'Save'}</button>
 <button class="btn btn-teal" style="flex:1;font-size:13px;" onclick="syncCoursesFromSheet()"> ${LANG==='th'?'ซิงค์คอร์ส':'Sync Courses'}</button>
 </div>
 <div id="courseSyncStatus" style="margin-top:8px;font-size:12px;color:var(--gray);"></div>
 </div>

 <div class="card">
 <div style="font-weight:800;color:var(--navy);margin-bottom:6px;"> App Info</div>
 <div style="font-size:12px;color:var(--gray);line-height:2;">Firebase: cch-academy (asia-southeast1)<br>Hosting: synder282061122.github.io/-cch-academy/</div>
 </div>`;
}

function saveSheetUrls(){
 const gasUrl = (document.getElementById('gasUrlInput')||{value:''}).value.trim();
 const sheetUrl = (document.getElementById('sheetUrlInput')||{value:''}).value.trim();
 const courseGasUrl = (document.getElementById('courseGasUrlInput')||{value:''}).value.trim();
 const courseSheetUrl = (document.getElementById('courseSheetUrlInput')||{value:''}).value.trim();
 if(gasUrl) localStorage.setItem('cch_gas_url', gasUrl);
 if(sheetUrl) localStorage.setItem('cch_sheet_url', sheetUrl);
 if(courseGasUrl) localStorage.setItem('cch_course_gas_url', courseGasUrl);
 if(courseSheetUrl) localStorage.setItem('cch_course_sheet_url', courseSheetUrl);
 showToast(t('save')+' ✓');
 renderPage();
}
...
function syncCoursesFromSheet(){
 const url = localStorage.getItem('cch_course_gas_url')||'';
 const statusEl = document.getElementById('courseSyncStatus');
 if(!url){ showToast(LANG==='th'?'กรุณาใส่ Apps Script URL ของคอร์สก่อน':'Please enter Course Apps Script URL'); return; }
 if(statusEl) statusEl.textContent = LANG==='th'?'กำลังซิงค์...':'Syncing...';
 fetch(url)
 .then(res=>res.json())
 .then(rows=>{
 const deptMap={}, courseMap={};
 rows.forEach((row,i)=>{
 if(!row.titleEN && !row.titleTH) return;
 const dk=(row.deptNameEN||'General').replace(/[^a-zA-Z0-9]/g,'_');
 if(!deptMap[dk]){ const ex=STATE.departments[dk]||{}; deptMap[dk]={nameTH:row.deptNameTH||dk,nameEN:row.deptNameEN||dk,icon:row.deptIcon||'',order:ex.order!==undefined?ex.order:Object.keys(deptMap).length}; }
 const ck='sh_'+(row.titleEN||row.titleTH||'').replace(/[^a-zA-Z0-9]/g,'').toLowerCase().slice(0,16)+'_'+i;
 const reqLv=row.requiredLevel?String(row.requiredLevel).split(',').map(s=>s.trim()):['ALL'];
 if(!courseMap[dk]) courseMap[dk]={};
 courseMap[dk][ck]={titleTH:row.titleTH||row.titleEN||'',titleEN:row.titleEN||row.titleTH||'',driveFileId:row.driveFileId||'',durationSec:parseInt(row.durationSec)||600,requiredLevel:reqLv,requiredDept:[dk],order:Object.keys(courseMap[dk]).length,active:true};
 });
 const updates={};
 Object.keys(deptMap).forEach(k=>{updates['departments/'+k]=deptMap[k];});
 Object.keys(courseMap).forEach(dk=>{Object.keys(courseMap[dk]).forEach(ck=>{updates['courses/'+dk+'/'+ck]=courseMap[dk][ck];});});
 return db.ref().update(updates);
 })
 .then(()=>{
 db.ref('meta/lastSheetSync').set(firebase.database.ServerValue.TIMESTAMP);
 const msg=(LANG==='th'?'ซิงค์คอร์สสำเร็จ':'Courses synced')+' ✓';
 if(document.getElementById('courseSyncStatus')) document.getElementById('courseSyncStatus').textContent=msg;
 showToast(msg);
 })
 .catch(err=>{if(document.getElementById('courseSyncStatus')) document.getElementById('courseSyncStatus').textContent='Error: '+err.message;});
}